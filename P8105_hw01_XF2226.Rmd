...
* The names of important variables in the penguins dataset are `r names(penguins)`. 
* The number of rows is `r nrow(penguins)` and the number of columns is `r ncol(penguins)`.
* The mean of the flipper length variable is `r mean(pull(penguins, flipper_length_mm))`.

```{r problem 2 - Make a scatterplot of flipper_length_mm (y) vs bill_length_mm (x)}
